...
def find_min_and_max(dict):
    ...
    for k,v in dict.items():
        ...
        if dict[k] > maximum:
            maximum = dict[k]
    return {minimum: rotate, maximum: touch}
...
#        time.sleep(0.01)
    ...
    while counter < 5:
        ...
        while True:
            ...
            print('Нашел ', int_r(rect/frame), ' черных кругов')
            break
    ...
    # Находим номера табличек с наименьшими и наибольшими значениями (если считать с конца)
    dict1 = find_min_and_max(circles)
    n_mini, n_maxi = dict1.keys()
    n_mini = counter - n_mini
    n_maxi = counter - n_maxi
    ...
    # Делаем соответсвующее действие
    dict1[m]()
    m = n_mini
    if m == n_maxi:
        m = n_maxi
