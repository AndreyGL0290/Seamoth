...
if __name__ == '__main__':    
    ...
    while True:
        _, frame1 = video1.read()
        cnt = find_contours(frame1, color_orange_day)

        try:
            center_point = get_center(frame1, cnt)
            power = stabilization(center_point, up, down)
            
            print(power)

            cv2.line(frame1, (0, up), (len(frame1[0]), up), (0, 0, 255), 3)
            cv2.line(frame1, (0, down), (len(frame1[0]), down), (0, 0, 255), 3)
